...
import { Runner, gatingFailures, orderProblems, parseArgs, renderSummary } from "../ci/self-hosted/lanes.mjs";

const repoRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "..");
const ciLite = fs.readFileSync(path.join(repoRoot, ".github", "workflows", "ci-lite.yml"), "utf8");
...
test("commands are static: no suite is ever narrowed to the diff", () => {
  for (const plan of plans()) {
    for (const run of allRuns(plan)) {
      assert.doesNotMatch(run, /\$\{\{|CI_AREA_|git diff|CHANGED|--changed|\brelated\b|--test-files/, run);
    }
  }
});
...
test("the complete suites run, not subsets", () => {
  ...
  assert.match(runs, /cargo llvm-cov --no-rustc-wrapper --manifest-path crates\/openhuman-app\/Cargo\.toml/);
});
...
test("untouched areas leave only the always-on gates", () => {
  ...
  const on = plan.lanes.flatMap((l) => l.checks.filter((c) => c.when).map((c) => `${l.name}:${c.name}`));
  assert.deepEqual(on, ["static:orch-ip-gate", "static:feature-forwarding", "static:module-pins", "static:submodule-monotonic"]);
  assert.deepEqual(hostedMatrix(plan).map((g) => g.group), ["checks"]);
});

test("hosted matrix only spins up groups with active lanes", () => {
  const docsOnly = hostedMatrix(buildPlan({ profile: "hosted", areas: { ...NONE, docs: true } }));
  assert.deepEqual(docsOnly.map((g) => g.group), ["checks"]);
  const core = hostedMatrix(buildPlan({ profile: "hosted", areas: { ...NONE, rustCore: true } }));
  assert.deepEqual(core.map((g) => g.group), ["checks", "rust-lint", "rust-cov"]);
  const pester = hostedMatrix(buildPlan({ profile: "hosted", areas: { ...NONE, installPs1: true } }));
...
test("ex63 needs a scratch dir and gives every Rust lane its own target dir under it", () => {
  assert.throws(() => buildPlan({ profile: "ex63", areas: ALL, env: {} }), /CI_SCRATCH_DIR/);
  ...
  assert.ok(buildPlan({ profile: "hosted", areas: ALL }).lanes.every((l) => !l.targetDir));
});
...
test("instrumented coverage never runs under a rustc wrapper", () => {
  for (const plan of plans()) {
    ...
    const tauriCov = plan.lanes.find((l) => l.name === "tauri").checks.find((c) => c.name === "tauri-coverage");
    assert.match(tauriCov.run, /^unset RUSTFLAGS RUSTC_WRAPPER/);
  }
});

test("area flags are read strictly from CI_AREA_*", () => {
  const areas = areasFromEnv({ CI_AREA_RUST_CORE: "true", CI_AREA_FRONTEND: "false", CI_AREA_DOCS: "1" });
...
test("argument parsing", () => {
  assert.deepEqual(parseArgs(["--profile", "hosted", "--lanes", "static, frontend", "--max-parallel", "2"]).lanes, ["static", "frontend"]);
  assert.throws(() => parseArgs([]), /--profile/);
  assert.throws(() => parseArgs(["--profile", "ex63", "--bogus"]), /unknown argument/);
});

function stubPlan() {
  const check = (name, run, extra = {}) => ({ name, run, when: true, needs: [], ...extra });
  return {
    profile: "hosted",
    lanes: [
      {
      ...
      { name: "b", active: true, checks: [check("waits-on-a", "echo ok", { needs: ["a:still-runs"] })] },
    ],
  };
}
...
test("runner: a failure never stops later checks, blocks its dependants, and gates the run", async () => {
  ...
  const status = Object.fromEntries(lanes.flatMap((l) => l.checks.map((c) => [`${l.name}:${c.name}`, c.status])));
