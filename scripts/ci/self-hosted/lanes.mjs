...
import { areasFromEnv, buildPlan, hostedMatrix, selectLanes, validatePlan } from "./lanes-plan.mjs";
...
export function parseArgs(argv) {
  const args = { profile: null, lanes: [], maxParallel: 0, out: "ci-out", dryRun: false, printMatrix: false };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--max-parallel") args.maxParallel = Number.parseInt(next(), 10);
  ...
  plan.lanes.forEach((lane, i) => {
    for (const c of lane.checks) {
      for (const need of c.needs) {
        ...
        if (!index.has(provider)) problems.push(`${lane.name}:${c.name} needs ${need}, whose lane is not selected`);
        else if (index.get(provider) > i) problems.push(`${lane.name}:${c.name} needs ${need}, whose lane is ordered after it`);
      }
    }
  });
  return problems;
...
export function renderSummary(results) {
  const rows = ["### CI lanes — per-check outcome", "", "| lane | check | outcome | time |", "| --- | --- | --- | --- |"];
  for (const lane of results.lanes) {
    for (const c of lane.checks) {
      const time = c.durationS == null ? "—" : `${Math.floor(c.durationS / 60)}m${String(c.durationS % 60).padStart(2, "0")}s`;
      const outcome = c.reportOnly && c.status === "failure" ? "failure (report-only)" : c.status;
      rows.push(`| ${lane.name} | ${c.name} | ${outcome} | ${time} |`);
    }
  }
  rows.push("", "`skipped` = area untouched; `blocked` = a check it needs did not succeed. Neither is a pass for the check itself.", "");
  return rows.join("\n");
}
...
export function gatingFailures(results) {
  ...
  for (const lane of results.lanes) {
    for (const c of lane.checks) {
      ...
      if (c.status === "failure" || c.status === "blocked" || c.status === "cancelled") failed.push(`${lane.name}:${c.name}`);
    }
  }
  return failed;
}
...
function sccacheStats() {
  const r = spawnSync("sccache", ["--show-stats", "--stats-format", "json"], { encoding: "utf8" });
...
class Runner {
  ...
  async runLane(lane) {
    ...
    const results = [];
    log(`lane ${lane.name}: start${lane.targetDir ? ` (target ${lane.targetDir})` : ""}`);
    for (const check of lane.checks) {
      const id = `${lane.name}:${check.name}`;
      const record = { name: check.name, status: "skipped", reportOnly: Boolean(check.reportOnly), exitCode: null, start: null, end: null, durationS: null };
      results.push(record);
      if (!check.when) {
      ...
      const needs = check.needs.map((n) => (n.includes(":") ? n : `${lane.name}:${n}`));
      const needStatuses = await Promise.all(needs.map((n) => this.done.get(n) ?? Promise.resolve("missing")));
      ...
      const beat = setInterval(() => {
        log(`lane ${lane.name}: ${check.name} still running (${Math.round((Date.now() - t0) / 60000)} min)`);
      }, HEARTBEAT_MS);
      ...
      record.status = this.cancelled ? "cancelled" : code === 0 ? "success" : "failure";
      logStream.write(`[ci][lanes] ${check.name}: ${record.status} (exit ${code}, ${record.durationS}s)\n`);
      log(`lane ${lane.name}: ${check.name} ${record.status} in ${record.durationS}s${record.reportOnly ? " (report-only)" : ""}`);
...
async function main() {
  ...
  const isPullRequest = (process.env.GITHUB_EVENT_NAME ?? "pull_request").startsWith("pull_request");
  const fullPlan = buildPlan({ profile: args.profile, areas, env: process.env, isPullRequest });
  ...
  log(`profile=${args.profile} areas=${Object.entries(areas).filter(([, v]) => v).map(([k]) => k).join(",") || "(none)"}`);
  for (const lane of plan.lanes) {
  ...
  if (args.dryRun) {
    for (const lane of plan.lanes) {
      for (const c of lane.checks.filter((x) => x.when)) console.log(`${lane.name}:${c.name}\t${c.run}`);
    }
    return 0;
  }
  ...
  if (args.profile === "ex63") spawnSync("sccache", ["--start-server"], { stdio: "ignore" });
  ...
  writeFileSync(join(out, "ci-timings.json"), `${JSON.stringify(results, null, 2)}\n`);
  ...
  if (failures.length > 0) {
    for (const f of failures) console.error(`::error::[ci][lanes] ${f} did not pass`);
    return 1;
  }
