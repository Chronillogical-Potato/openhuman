...
/**
 * Textual re-check of an unused flag. Returns "attribute/macro use" if the
 * crate name (with `-` folded to `_`) appears anywhere in the package's
 * sources, `build.rs`, benches or tests; "none" otherwise.
 */
function textualUse(dir, dep) {
  if (!dir || !fs.existsSync(dir)) return "unknown";
  const ident = dep.replace(/-/g, "_");
  try {
    // -l: stop at the first file per match; -r over the package dir only.
    const out = execFileSync(
      "grep",
      ["-rlE", `\\b${ident}\\b`, "--include=*.rs", "--exclude-dir=target", dir],
      { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] },
    );
    return out.trim() ? "attribute/macro use" : "none";
  } catch (err) {
    // grep exits 1 when nothing matched.
    if (err.status === 1) return "none";
    return "unknown";
  }
}
...
function unusedFor(target, data) {
  ...
  for (const u of data.dependencies.unused) {
    ...
    rows.push({
      ...
      evidence,
      verdict: evidence === "none" ? "remove" : "verify",
    });
  }
  return rows.sort(
    (a, b) =>
...
function renderMarkdown(summary, n) {
  ...
  push(
    `Verdict ${code("remove")}: the crate name appears nowhere in the package's ${code("*.rs")} files (not even inside an attribute) — delete the line from ${code("Cargo.toml")} and build.`,
    `Verdict ${code("verify")}: tinyanalyzer saw no ${code("use")}/path, but the name does occur textually (usually ${code("#[derive(crate::Trait)]")}) — probably a keeper; confirm by hand.`,
  ...
  if (unusedRows.length === 0) {
    ...
    for (const u of unusedRows) {
      push(
        `| ${u.target} | ${u.package} | ${code(u.dependency)} | ${u.kinds.join(", ")} | ${u.version ?? "—"} | ${u.exclusive_count ?? "—"} | ${u.verdict === "remove" ? "**remove**" : `verify (${u.evidence})`} |`,
      );
    }
  }
