...
A **memory source** is a configured connector that feeds the [Memory Tree](memory-tree.md). Where the tree owns _"how do I store and summarize?"_, the `memory_sources` domain (`crates/openhuman-core/src/memory/sources/`) owns the upstream question: **"what feeds my memory?"** It is a typed registry of connectors, persisted in `config.toml` under `[[memory_sources]]`, with CRUD at runtime, a uniform reader abstraction, per-source sync status, and the `openhuman.memory_sources_*` RPC surface.
...
Every source is a single flat `MemorySourceEntry` (`sources/types.rs`, engine-owned under `vendor/tinymemory`) whose `kind` discriminator (the `SourceKind` enum) decides which fields are required. Validation is enforced at add/update time by `validate()`, not the type system. The kinds:
...
Sources are CRUD-ed through the `memory_sources` controllers (`crates/openhuman-core/src/memory/sources/schemas.rs` → `rpc.rs`), namespace `openhuman.memory_sources_*`:
