...
Speech is configured separately from the LLM workload fields: STT and TTS routing strings live under `voice_providers` (Settings → Voice), not under `*_provider`. There is no local STT engine: speech-to-text is either the hosted proxy or a third-party API you supply a key for, while TTS keeps a local option in Piper.
