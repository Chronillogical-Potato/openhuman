...
- [ ] **Home connectivity chip reflects the core's hosted link** — Sign in on a desktop build and wait for the chip on Home to read green "Connected". Block the hosted backend for the core for about a minute (e.g. add `127.0.0.1 api.tinyhumans.ai` to the hosts file, or drop the VPN route). Expected: the chip turns amber "Disconnected" within ~30 s while chat keeps working, and returns to green within ~5 s of restoring the route. In `~/.openhuman/logs/openhuman.<date>.log` the `[socket] Connection failed (attempt N/5)` lines are never more than ~10 s plus the backoff apart, and recovery logs `[socket] Reconnected after …` (#6256).
