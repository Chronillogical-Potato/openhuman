...
| --- | --- |
| **A — direct imports and facade purge** | Import `tinytools-agent` directly; delete `dispatcher.rs`, `pformat.rs`, `harness/parse.rs`, `harness/instructions.rs`, `harness/tool_filter.rs`, `context/prompt.rs`, `registry/tools.rs`, the handoff wrapper, and behavior-free graph re-exports. |
...
`tinyagents_harness::tool_calling`, OpenHuman `agent::dispatcher`,
`agent::pformat`, `harness::parse`, and any forwarding definitions.
...
| `dispatcher.rs` | Delete | Use `tinytools-agent::dialect` directly. |
