...
/// [`MemoryTree::flavour_profile`](crate::memory::api::provider::MemoryTree::flavour_profile)
...
/// (`OpenHumanMemory::flavour` in `crate::flows::tinyflows::memory_adapter`)
...
pub(crate) async fn lookup_flavour(
    ...
    let guard = crate::memory::binding::for_config(config)?.guard();
