...
//! toolchain.
//!
//! Sibling to [`crate::tools::impl::system::shell::ShellTool`]: same
//! security gates, same env hygiene, but the command is pinned to the `node`
//! binary resolved by
//! [`crate::runtime::javascript::NodeBootstrap`].
//!
//! Two input modes:
//!
...
pub struct NodeExecTool {
    ...
    pool_cfg: crate::config::RuntimePoolConfig,
...
impl NodeExecTool {
    async fn execute_in_context(
        ...
        let explicit_timeout = crate::tools::timeout::explicit_call_timeout_duration(
...
impl NodeExecTool {
    ...
    async fn try_pool_inline(
        ...
        if !crate::runtime::pool::node::enabled(&self.pool_cfg) {
            return None;
        }
        ...
        match crate::runtime::pool::node::run_inline(
        ...
        .await
        {
            ...
            Err(crate::runtime::pool::PoolRunError::PreDispatch(error)) => {
                tracing::warn!(
                    error = %error,
            ...
            Err(crate::runtime::pool::PoolRunError::Saturated) => {
            ...
            Err(crate::runtime::pool::PoolRunError::PostDispatch(error)) => {
                tracing::warn!(
                    error = %error,
...
fn pool_outcome_to_result(
    outcome: crate::runtime::pool::PoolExecOutcome,
    timeout: Option<Duration>,
) -> ToolResult {
