    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // URL validation / DNS so nothing leaves the device.
        {
            ...
            if let Some(msg) = crate::security::egress::local_only_tool_block(
                &crate::security::egress::EgressDescriptor::network_fetch(host.clone()),
            ) {
        ...
        // downloads.
        {
            use crate::security::egress::{DataKind, EgressDescriptor};
            let host = reqwest::Url::parse(&url)
                .ok()
                .and_then(|u| u.host_str().map(str::to_string))
                .unwrap_or_else(|| "unknown".to_string());
            ...
            crate::security::egress::emit_external_transfer(desc);
        }
        ...
        let builder = crate::config::apply_runtime_proxy_to_builder(builder, "tool.curl");
