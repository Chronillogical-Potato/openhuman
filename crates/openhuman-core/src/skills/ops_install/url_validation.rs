//! URL normalization and SSRF-safety validation for skill install URLs.

use super::super::ops_types::{WorkflowFrontmatter, MAX_NAME_LEN};
    ...
    for addr in addrs {
        ...
        match ip {
            std::net::IpAddr::V4(v4) => {
                if crate::tools::is_non_global_v4(v4) {
                    tracing::warn!(
                        host = %host,
                        resolved = %v4,
            ...
            std::net::IpAddr::V6(v6) => {
                if crate::tools::is_non_global_v6(v6) {
                    tracing::warn!(
                        host = %host,
                        resolved = %v6,
...
fn is_blocked_install_host(host: &str) -> bool {
    crate::tools::is_private_or_local_host(host)
}
