...
fn list_agent_definitions_tool_description_has_no_stale_followup_language() {
    ...
    let description = ListAgentDefinitionsTool::new().description().to_string();

    for banned in ["is a follow-up", "for now"] {
        assert!(
            !description.contains(banned),
            "list_agent_definitions description must not carry the stale \
    ...
    assert!(
        ...
        "list_agent_definitions description must describe agent_ref as running \
