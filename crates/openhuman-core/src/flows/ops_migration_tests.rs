...
/// Regression cases for migration errors that identify the invalid collection
/// member rather than reporting an ambiguous top-level deserialization error.
#[test]
...
fn migrate_and_deserialize_graph_falls_back_when_no_member_is_at_fault() {
    let err = migrate_and_deserialize_graph(json!({
        "name": "valid",
