...
Capability {
        ...
        name: "Declare MCP Servers in mcp.json",
        domain: "channels",
        category: CapabilityCategory::Channels,
        description: "Declare MCP servers — local stdio subprocesses (`command`/`args`/`env`) and hosted HTTP-remote servers (`url`/`headers`) — in one mcp.json document, in the shape other MCP clients use. Credentials are write-only: stored encrypted on save and never read back.",
        how_to: "Connections > MCP Servers > mcp.json > paste a server block > Save",
        status: CapabilityStatus::Beta,
