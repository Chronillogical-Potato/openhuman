...
//! [`crate::platform::socket::SocketManager`] — the transport, handshake auth
...
//!
//! Down (backend → openhuman), handled in [`crate::platform::socket::event_handlers`]:
...
pub fn emit_register_agents() {
    let agents: Vec<AgentDescriptor> = crate::agent::registry::default_agents()
        .into_iter()
...
async fn describe_self(
    ...
    if let Some(entry) = crate::agent::registry::default_agents()
        .into_iter()
...
fn emit<T: serde::Serialize>(event: &str, payload: T) {
    ...
    let Some(mgr) = crate::platform::socket::global_socket_manager() else {
...
async fn emit_awaited<T: serde::Serialize>(event: &str, payload: T) -> bool {
    ...
    let Some(manager) = crate::platform::socket::global_socket_manager() else {
        log::debug!("[medulla] no socket manager — dropping {event}");
        return false;
    };
