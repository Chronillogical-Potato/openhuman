...
- Track connection status / socket id / last user-visible error / loop liveness (`SharedState::loop_active`, `SocketManager::is_loop_active`, surfaced as `connectivity_diag.socket_loop_active`) in shared state and expose it via RPC. Between attempts the loop reports `Reconnecting`; `Disconnected` means the loop is not running (signed out, session expired, shutdown), which is what the frontend's connectivity chip relies on to show "down, retrying" and stay quiet for "never wanted" (#6256).
