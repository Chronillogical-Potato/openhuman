...
fn optional_format(
    ...
    key: &str,
) -> anyhow::Result<Option<crate::voice::audio_toolkit::AudioFormat>> {
    let Some(raw) = args.get(key).and_then(|v| v.as_str()) else {
        return Ok(None);
    };
    match raw.trim() {
        "mp3" => Ok(Some(crate::voice::audio_toolkit::AudioFormat::Mp3)),
        "wav" => Ok(Some(crate::voice::audio_toolkit::AudioFormat::Wav)),
