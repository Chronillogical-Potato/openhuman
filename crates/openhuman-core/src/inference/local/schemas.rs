...
fn handle_agent_chat_simple(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::inference::local::ops::agent_chat_simple(
                &config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_local_ai_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::local::ops::local_ai_transcribe(&config, p.audio_path.trim()).await?,
        )
    })
}
...
fn handle_local_ai_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::local::ops::local_ai_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
            )
...
fn handle_local_ai_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::local::ops::local_ai_tts(&config, &p.text, p.output_path.as_deref())
                .await?,
        )
    })
}
...
fn handle_local_ai_assets_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::inference::local::ops::local_ai_assets_status(&config).await?)
    })
}
...
fn handle_local_ai_downloads_progress(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::inference::local::ops::local_ai_downloads_progress(&config).await?)
    })
}
...
fn handle_local_ai_download_asset(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::local::ops::local_ai_download_asset(&config, p.capability.trim())
                .await?,
        )
    })
}
...
fn handle_local_ai_test_connection(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let result =
            crate::inference::local::service::ollama_admin::test_ollama_connection(&p.url).await?;
