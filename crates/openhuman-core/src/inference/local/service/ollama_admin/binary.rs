...
use tinyinference_local::install::{find_system_ollama_binary, run_ollama_install_script};
use tinyinference_local::process::apply_no_window;

use super::super::LocalAiService;

impl LocalAiService {
    pub(in crate::inference::local::service) async fn resolve_or_install_ollama_binary(
        &self,
        config: &Config,
    ...
    pub(in crate::inference::local::service) async fn command_works(&self, command: &Path) -> bool {
    ...
    pub(in crate::inference::local::service) async fn download_and_install_ollama(
        &self,
        config: &Config,
    ) -> Result<(), String> {
        let install_dir = crate::inference::paths::workspace_ollama_dir(config);
        ...
        if tinyinference_local::install::is_ollama_installer_running() {
            log::info!(
            ...
            while tinyinference_local::install::is_ollama_installer_running() {
