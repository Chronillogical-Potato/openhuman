...
/// Stable fragment emitted when provider resolution has no configured model id.
pub const NO_MODEL_CONFIGURED_ANCHOR: &str = "resolved to an empty model id";
...
pub fn is_provider_config_rejection_message(body: &str) -> bool {
    ...
    if lower.contains("reliability.model_fallbacks")
        || lower.contains("may not be available on your provider")
    {
        let transient_attempt = [
            "timed out",
            "timeout",
