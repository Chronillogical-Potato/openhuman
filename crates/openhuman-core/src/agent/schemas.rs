...
fn handle_list_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let defs = crate::agent::library::list_definition_metadata().await?;
...
fn handle_reload_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        ...
        let already_loaded = crate::agent::harness::AgentDefinitionRegistry::global().is_some();
        Ok(serde_json::json!({
            "status": "noop",
    ...
    Box::pin(async move {
        ...
        let envelope = match p.source.as_str() {
            "composio" => {
                ...
                crate::agent::triage::TriggerEnvelope::from_composio(
            ...
            "webhook" => {
                ...
                crate::agent::triage::TriggerEnvelope::from_webhook(
            ...
            "cron" => {
                ...
                let output = p
                    ...
                    .unwrap_or(job_name);
                crate::agent::triage::TriggerEnvelope::from_cron(job_id, job_name, output)
            }
            "external" => {
                ...
                crate::agent::triage::TriggerEnvelope::from_external(caller_id, reason, p.payload)
            }
            other => {
        ...
        let outcome = crate::agent::triage::run_triage(&envelope)
            .await
        ...
        match outcome {
            crate::agent::triage::TriageOutcome::Decision(run) => {
                if !dry_run {
                    ...
                    let origin = crate::agent::triage::local_trigger_origin();
...
fn handle_graph_topologies(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let reports = crate::agent::tinyagents::all_graph_topologies();
        let agents = crate::agent::registry::agents::load_builtins()
...
fn local_catalog_models_from_config(
    ...
    config
        ...
        .filter_map(|entry| {
            ...
            Some(crate::platform::cost::catalog::LocalCatalogModel {
...
fn handle_registry_snapshot(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        ...
        let catalog = crate::platform::cost::catalog::unified_model_catalog(&local_models);
        ...
        let agent_defs = crate::agent::registry::agents::load_builtins()
