...
/// Typed harness dispatch for personality delegation.
pub(crate) struct DelegateToPersonalityDispatch {
    tool: Arc<dyn Tool>,
}

impl DelegateToPersonalityDispatch {
...
impl ToolDispatch<(), crate::agent::tinyagents::host::OpenHumanRunContext>
    for DelegateToPersonalityDispatch
{
    ...
    async fn execute(
        ...
        execute_delegate_to_personality(arguments, Some(&context), parent.data.child()).await
    }
}
...
impl Tool for DelegateToPersonalityTool {
    ...
    async fn execute_with_context(
        ...
        execute_delegate_to_personality(args, tool_context, run_context).await
    }
}

/// Execute personality delegation with an explicit child run carrier.
pub(crate) async fn execute_delegate_to_personality(
    args: serde_json::Value,
    tool_context: Option<&dyn ToolRunContext>,
    ...
    let parent_ctx = match run_context.parent.clone() {
        Some(ctx) => ctx,
        None => {
            return Ok(ToolResult::error(
