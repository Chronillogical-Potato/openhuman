...
use tinyinference_core::message::{ContentBlock, Message as TaMessage};
use tinyinference_core::model::{ModelResponse, SegmentRole};
...
#[async_trait]
impl crate::agent::hooks::ToolHook for RecordingToolHook {
    fn name(&self) -> &str {
        self.name
    }
    async fn before_tool(
        &self,
        context: &crate::agent::hooks::ToolHookContext,
    ) -> anyhow::Result<()> {
        self.pre
            .lock()
            .unwrap()
