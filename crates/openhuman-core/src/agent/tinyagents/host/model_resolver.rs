...
//! inference domain — `crate::inference::provider` (the per-role
...
impl OpenHumanModelResolver {
    ...
    /// `crate::agent::tinyagents::TurnModelSource::build` (see
    /// `build_turn_models_crate` in `crates/openhuman-core/src/tinyagents/mod.rs`). That is a
