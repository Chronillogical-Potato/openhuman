...
//! [`crate::memory`] directly. Instead the crate declares
//! [`AgentMemory`] and this module is the single place OpenHuman's memory
//! domains meet it.
//!
//! # Domains adapted
//!
//! - [`crate::memory`] — the `Memory` trait, `MemoryEntry`,
//!   `MemoryCategory`, `MemoryTaint`, `RecallOpts`.
//! - `crate::agent::tinyagents::retriever::recall_through_facade` — the
...
//!   `crate::flows::memory_tools` uses to keep a flow inside its
...
use crate::memory::{Memory, MemoryCategory, MemoryEntry, MemoryTaint, RecallOpts};
...
impl AgentMemory for OpenHumanAgentMemory {
    ...
    async fn recall(&self, req: RecallRequest) -> TaResult<Vec<MemoryItem>> {
        ...
        let entries = crate::agent::tinyagents::retriever::recall_through_facade(
            self.memory.as_ref(),
            &req.query,
            limit,
            opts,
