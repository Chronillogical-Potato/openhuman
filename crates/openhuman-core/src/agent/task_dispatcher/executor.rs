...
pub(super) fn resolve_executor(workspace_dir: &Path, assigned: Option<&str>) -> ResolvedExecutor {
    ...
    #[cfg(feature = "skills")]
    if let Some(skill) = crate::skills::registry::get_workflow(workspace_dir, handle) {
...
pub(super) async fn run_autonomous(
    ...
    if let Some(thread_id) = session_thread_id.as_deref() {
        ...
        crate::web_chat::spawn_progress_bridge(
            ...
            thread_id.to_string(),
            run_id.to_string(),
            crate::threads::turn_state::TurnStateStore::new(workspace_dir.clone()),
    ...
    if let Some(thread_id) = session_thread_id.as_deref() {
        ...
        match &result {
            Ok(response) => {
                ...
                // tracker.
                crate::web_chat::presentation::deliver_response_single_bubble(
