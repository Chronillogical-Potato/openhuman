...
fn a_restart_with_no_integrations_rebuilds_the_recorded_actions_as_deferred() {
    ...
    let rebuilt = rehydrate_integration_actions(&recorded, &[]);
...
fn a_live_action_is_not_rebuilt_from_the_record() {
    ...
    let live: Vec<Box<dyn Tool>> = rehydrate_integration_actions(&[spec("GMAIL_SEND_EMAIL")], &[]);
    let rebuilt = rehydrate_integration_actions(&recorded, &live);
    let names: Vec<&str> = rebuilt.iter().map(|tool| tool.name()).collect();
...
fn a_rebuilt_declaration_is_byte_identical_to_the_recorded_one() {
    let first = rehydrate_integration_actions(&[spec("GMAIL_SEND_EMAIL")], &[]);
    let recorded = vec![first[0].spec()];
    let second = rehydrate_integration_actions(&recorded, &[]);
    assert_eq!(
