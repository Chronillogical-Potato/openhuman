...
/// set, and the callable-name allowlist. See [`OpenHumanSessionHost::turn_tool_sets`].
type TurnToolSets = (
...
impl OpenHumanSessionHost {
    ...
    /// [`OpenHumanSessionHost::synthesized_tools`] for why they are not one `Arc`.
    ///
    ...
    pub(super) fn refresh_delegation_tools_from_cached_integrations(
        ...
        let Some(cache_view) = crate::integrations::composio::cached_active_integrations(cfg)
        else {
            return false;
        };

        let new_hash = crate::integrations::composio::connected_set_hash(&cache_view);
    ...
    pub(in super::super) fn refresh_workflows(&mut self, trigger: &str) -> bool {
        let id_of = |w: &crate::skills::Workflow| -> String {
