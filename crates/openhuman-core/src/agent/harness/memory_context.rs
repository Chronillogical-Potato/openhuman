...
pub(crate) async fn build_context_for_thread(
    ...
    if let Ok(entries) = crate::agent::tinyagents::retriever::recall_through_facade(
        mem,
        user_msg,
        5,
        crate::memory::RecallOpts::default(),
    )
    .await
    {
    ...
    .await
    {
        ...
        if !cross.is_empty() {
            ...
            context.push_str(crate::memory::agent::memory_loader::CROSS_CHAT_HEADER);
            for entry in &cross {
                let prov = entry
                    .session_id
                    .as_deref()
