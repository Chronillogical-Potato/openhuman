...
fn refresh_workflows_picks_up_skill_installed_on_disk() {
    ...
    use crate::skills::ops_types::{SKILL_MD, TRUST_MARKER};
...
async fn turn_without_tools_returns_text() {
    ...
    let provider = Arc::new(MockProvider {
        responses: Mutex::new(vec![crate::inference::provider::ChatResponse {
...
async fn last_turn_usage_is_public_and_non_draining() {
    ...
    let provider = Arc::new(MockProvider {
        responses: Mutex::new(vec![crate::inference::provider::ChatResponse {
            text: Some("hello".into()),
            tool_calls: vec![],
            usage: Some(crate::inference::provider::UsageInfo {
                input_tokens: 123,
                output_tokens: 45,
                context_window: 8000,
