...
fn seed_resume_from_thread_transcript_is_noop_on_warm_agent() {
    let mut agent = build_minimal_agent_with_definition_name(Some("orchestrator"));
    agent.cached_transcript_messages = Some(vec![crate::agent::messages::ChatMessage::system(
        "warm prefix",
    )]);
...
async fn fake_locator_substitutes_the_whole_turn_path() {
    ...
    // (3) The write.
    let turn = vec![
        crate::agent::messages::ChatMessage::user("live question"),
        crate::agent::messages::ChatMessage::assistant("live answer"),
    ];
