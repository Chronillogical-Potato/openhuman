//! The tinyagents-backed chat turn driver (`run_turn_via_tinyagents_session`).
//!
//! Converts the frozen history to provider messages, runs the harness loop over
...
impl Agent {
    ...
    pub(super) async fn run_turn_via_tinyagents_session_inner(
        ...
        let messages = crate::agent::multimodal::prepare_messages_for_provider(
            &messages,
            &multimodal,
            &multimodal_files,
        )
