...
async fn fetch_learned_context_returns_general_prefs_when_explicit_flag_on_learning_off() {
    ...
    mem.store(
        crate::memory::preferences::USER_PREF_GENERAL_NAMESPACE,
        "package_manager",
...
fn assistant_message_has_tool_calls_rejects_envelope_without_content_field() {
    ...
    // is not the envelope the canonical native dialect emits.
    let body = serde_json::json!({
        "tool_calls": [{ "id": "tc-1", "name": "shell", "arguments": "{}" }]
    })
    .to_string();
...
use crate::agent::experience::ops::DriverMemory;
use crate::memory::api::provider::MemoryProvider;
use crate::memory::guard::test_support::{namespace_hit, namespace_summary, RecordingProvider};
use crate::memory::preferences::USER_PREF_SITUATIONAL_NAMESPACE;
