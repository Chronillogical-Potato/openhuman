...
impl ChatModel<()> for StaticModel {
    async fn invoke(
        ...
        match response {
            Ok(response) => Ok(
                crate::agent::tinyagents::model::native_model_response_for_request(
                    &response, &request,
                ),
            ),
            Err(error) => Err(tinyinference_llm::Error::Model(error.to_string())),
        }
    }
...
fn helper_paths_cover_no_overlap_native_calls_and_truncation() {
    ...
    let native_calls = vec![crate::inference::provider::ToolCall {
        id: "native-1".into(),
        name: "echo".into(),
        arguments: "{}".into(),
        extra_content: None,
