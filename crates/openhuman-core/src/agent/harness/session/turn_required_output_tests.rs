...
async fn turn_override_suppress_memory_agent_skips_memory_trigger() {
    ...
    let mut agent = Agent::builder()
        ...
        .trigger_memory_agent(crate::agent::harness::definition::TriggerMemoryAgent::Always)
        .build()
        .unwrap();
...
async fn turn_accepts_valid_required_output_without_repair() {
    ...
    let config = crate::config::AgentConfig {
        max_tool_iterations: 3,
        max_history_messages: 10,
        required_output: Some(crate::config::RequiredOutputContract {
...
async fn turn_synthesizes_required_output_when_reprompt_also_omits() {
    ...
    let config = crate::config::AgentConfig {
        max_tool_iterations: 3,
        max_history_messages: 10,
        required_output: Some(crate::config::RequiredOutputContract::new("thoughts")),
        ..crate::config::AgentConfig::default()
    };
...
async fn turn_appends_required_output_block_when_streamed_to_preserve_consistency() {
    ...
    let drain = tokio::spawn(async move {
        while let Some(progress) = rx.recv().await {
            if let crate::agent::progress::AgentProgress::TextDelta { delta, .. } = progress {
