...
fn tool_results(id: &str) -> ConversationMessage {
    use crate::agent::messages::ToolResultMessage;
...
fn user_chat(text: &str) -> ConversationMessage {
    ConversationMessage::Chat(crate::agent::messages::ChatMessage::user(text))
}

fn assistant_chat(text: &str) -> ConversationMessage {
    ConversationMessage::Chat(crate::agent::messages::ChatMessage::assistant(text))
}
...
fn assistant_tool_calls_multi(ids: &[&str]) -> ConversationMessage {
    ConversationMessage::AssistantToolCalls {
        ...
        tool_calls: ids
            .iter()
            .map(|id| crate::inference::provider::ToolCall {
                id: (*id).into(),
                name: "shell".into(),
                arguments: "{}".into(),
                extra_content: None,
...
fn to_provider_messages_treats_unrecognized_role_as_user() {
    let dispatcher = XmlToolDispatcher;
    let mut developer_message = crate::agent::messages::ChatMessage::user("hi");
...
fn serializing_the_same_history_twice_is_byte_identical() {
    ...
    let render = |messages: &[crate::agent::messages::ChatMessage]| {
        messages
            .iter()
