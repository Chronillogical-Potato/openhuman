...
- `crate::inference::provider::create_chat_model_from_string_with_model_id` (profile summarisation) and `crate::inference::local::global` (local reflection route); `ReflectionHook` also accepts an optional `tinyinference_core::model::ChatModel` for the cloud fallback.
