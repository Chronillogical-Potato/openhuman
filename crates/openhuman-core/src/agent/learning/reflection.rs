...
use crate::agent::learning::candidate::{self, CueFamily, EvidenceRef, FacetClass};
...
pub struct ReflectionHook {
    ...
    provider: Option<Arc<dyn tinyinference_core::model::ChatModel<()>>>,
...
async fn invoke_cloud_reflection(
    provider: &Arc<dyn tinyinference_core::model::ChatModel<()>>,
    prompt: &str,
...
impl ReflectionHook {
    ...
    fn emit_candidates_from_reflection(output: &ReflectionOutput, episodic_id: i64) {
        ...
        for (n, pref) in output.user_preferences.iter().enumerate() {
            ...
            buf.push(crate::agent::learning::candidate::LearningCandidate {
...
impl PostTurnHook for ReflectionHook {
    ...
    async fn on_turn_complete(&self, ctx: &TurnContext) -> anyhow::Result<()> {
        ...
        if self.config.enabled {
            ...
            let episodic_id = crate::agent::learning::candidate::global().len() as i64;
