...
pub enum SubagentRunStatus {
    ...
    /// Cooperative cancellation won the subagent lifecycle. This is distinct
    /// from an incomplete run so callers never publish a completed result for
    /// work the user or host explicitly stopped.
    Cancelled,
}
...
pub struct SubagentRunOutcome {
    ...
    /// The neutral persistence boundary that produced this outcome. Event and
    /// progress publishers must gate terminal effects on its committed variants
    /// so replayed durable outcomes are not announced a second time.
    pub persistence_disposition: tinyagents_orchestration::subagent::SubagentPersistenceDisposition,
}

impl SubagentRunOutcome {
    /// Whether this invocation owns lifecycle-completion event/progress
    /// effects after a successful durable commit.
    pub const fn should_emit_lifecycle_effects(&self) -> bool {
        self.persistence_disposition.should_emit_host_effects()
    }
}
...
pub struct SubagentCheckpointData {
    /// The original neutral key, recovered for continuation rather than
    /// re-derived from a fresh child context.
    #[serde(default)]
    pub task_key: Option<tinyagents_orchestration::subagent::SubagentTaskKey>,
    pub task_id: String,
    pub agent_id: String,
    pub worker_thread_id: Option<String>,
    ...
    #[serde(default)]
    pub resume_metadata: std::collections::BTreeMap<String, String>,
    /// Visible outcome fields retained with a pause so an observer that loses
    /// the lifecycle compare-and-swap can return the durable winner exactly.
    #[serde(default)]
    pub output: String,
    #[serde(default)]
    pub iterations: usize,
    ...
    #[serde(default)]
    pub artifact_paths: Vec<String>,
    /// Wall-clock duration of the durable winner. Kept with pause data so a
    /// duplicate observer can reconstruct the original host outcome without
    /// substituting its own elapsed time.
    #[serde(default)]
    pub elapsed_ms: u64,
}
