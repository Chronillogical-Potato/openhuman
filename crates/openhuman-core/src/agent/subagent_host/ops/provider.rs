...
/// [`crate::integrations::composio::ComposioClient`] so the live
/// `composio.mode` toggle is honoured per execute — see
/// [`crate::integrations::composio::ComposioActionTool`] and issue #1710.
///
/// ## Tool caching (#5119)
///
        ...
        // ContractGate) for repeated resolve calls on the same slug.
        {
            ...
            if let Some(cached) = cache.get(name) {
                tracing::trace!(
                    ...
                    slug = %name,
                    "[subagent_host] returning cached composio tool"
                );
    ...
    fn find_action(&self, name: &str) -> Option<&crate::agent::prompts::ConnectedIntegrationTool> {
            ...
            .find(|a| a.name.eq_ignore_ascii_case(name))
        {
            tracing::debug!(
                requested = %name,
                matched = %action.name,
                "[subagent_host] resolved tool by case-insensitive match"
            );
            return Some(action);
        }
            ...
            if let Some(action) = matches.next() {
                if matches.next().is_none() {
                    tracing::info!(
                        requested = %name,
                        matched = %action.name,
                        "[subagent_host] resolved tool by normalized-slug match"
                    );
                    return Some(action);
                }
                ...
                tracing::warn!(
                    requested = %name,
                    norm = %norm,
                    "[subagent_host] ambiguous normalized-slug match — multiple actions resolve to the same slug; not resolving"
                );
                return None;
            }
                ...
                if let Some(action) = prefix_matches.next() {
                    if prefix_matches.next().is_none() {
                        tracing::info!(
                            requested = %name,
                            matched = %action.name,
                            "[subagent_host] resolved tool by unique prefix/superstring match"
                        );
                        return Some(action);
                    }
                    tracing::warn!(
                        requested = %name,
                        norm = %norm,
                        "[subagent_host] ambiguous prefix/superstring match — multiple actions share the slug prefix; not resolving"
                    );
                }
            }
        }
