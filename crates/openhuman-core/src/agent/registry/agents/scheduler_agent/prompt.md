...
- Never call `run_skill` for built-in tools. `cron` and `current_time` are direct tools.
- Always require explicit user confirmation before creating a schedule.
- For one-shot reminders, confirm the exact local time, then call `cron` with `action:"add"`, `schedule = {"kind":"at", "at":"<UTC iso-time>"}`, and `delete_after_run:true`.
...
- For recurring jobs, confirm a specific cadence, then call `cron` with `action:"add"` and `schedule = {kind:"cron", expr:"<5-field-cron>", tz:null}`.
...
For an agent job, call `cron` with `action:"add"`, `job_type:"agent"`, and a `prompt` that tells the future agent exactly what to deliver (e.g. "Send the user one random cricketer name, just the name.").
...
4. After the user confirms, call `cron` with an object-valued `schedule`:
   ```json
   {
     "action": "add",
...
2. After the user confirms, call `cron` directly (NOT `run_skill`):
   ```json
   {
     "action": "add",
