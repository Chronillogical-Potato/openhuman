...
impl WebSearchTool {
    ...
    fn resolve_client(&self) -> Option<Arc<IntegrationClient>> {
        let fresh = self
            .root_config
            .as_deref()
            .and_then(crate::integrations::build_client);
    ...
    fn parse_parallel_results(
        ...
        for (i, result) in results.iter().take(self.max_results).enumerate() {
            ...
            if let Some(first) = result.excerpts.first() {
                ...
                if !excerpt.is_empty() {
                    let truncated = crate::util::truncate_with_ellipsis(excerpt, 500);
    ...
    fn render_results_markdown(
        ...
        for r in results.iter().take(self.max_results) {
            ...
            if let Some(first) = r.excerpts.first() {
                let excerpt = first.trim();
                if !excerpt.is_empty() {
                    let truncated = crate::util::truncate_with_suffix(excerpt, 500, "…");
