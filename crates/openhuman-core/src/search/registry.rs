...
fn build_backend_search_tools(root_config: &Config) -> Vec<Box<dyn Tool>> {
    ...
    if root_config.integrations.tinyfish.is_active() {
        tools.push(Box::new(crate::search::tools::TinyFishSearchTool::new(
            Arc::clone(&client),
        )));
        tools.push(Box::new(crate::search::tools::TinyFishFetchTool::new(
            Arc::clone(&client),
        )));
        tools.push(Box::new(crate::search::tools::TinyFishAgentRunTool::new(
            Arc::clone(&client),
        )));
