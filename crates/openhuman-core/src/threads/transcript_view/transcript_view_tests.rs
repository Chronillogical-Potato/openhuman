...
fn prompt_guided_results_without_ids_pair_in_issue_order() {
    ...
    assert_eq!(outline[4], "tool:call-grep:grep:Success:config.toml:3: setting = on");
...
fn native_turn_projection_is_unchanged() {
    let dir = TempDir::new().unwrap();
    let path = write_raw(dir.path(), "302_orchestrator", "thr_n", &full_turn_body());
    let display = read_transcript_display(&path).unwrap();
    assert_eq!(
        outline(&project_records(&display.records)),
        vec![
            "user:Current Date & Time: 2026-07-21 09:00:00 UTC\n\nWhat's the weather in NYC?"
                .to_string(),
