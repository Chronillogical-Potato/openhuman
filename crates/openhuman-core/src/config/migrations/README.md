...
| `crates/openhuman-core/src/config/migrations/retire_chat_v1_model.rs` | **2→3.** Legacy `chat-v1` migration hook retained for schema-version progression; a no-op (the slug itself is retired by 12→13). Pure in-memory. |
...
| `crates/openhuman-core/src/config/migrations/normalize_default_model_tier.rs` | **7→8.** Rewrites a persisted `default_model` of `reasoning-v1` or `reasoning-quick-v1` to the managed default model; every other value (custom/BYOK ids, `None`) is left untouched. Pure in-memory. |
...
| `crates/openhuman-core/src/config/migrations/retire_subconscious_medulla.rs` | **11→12.** Retires the Medulla subconscious engine selection. |
...
- **Exact `==` gating (except 0→1).** Steps 1→2 through 12→13 guard on `schema_version == N`, not `< N+1`, so a failed earlier migration cannot be silently skipped. The first step uses `< 1`.
