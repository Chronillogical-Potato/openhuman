...
fn resolve_chat_model(
    ...
    let (model, _) = crate::inference::provider::create_chat_model_with_model_id(
        role,
        config,
...
impl ComposioCallbacks {
            ...
            .map_err(|e| method_error(format!("create_composio_client: {e:#}")))?
        {
            ...
            ComposioClientKind::Direct(direct) => {
                direct_list_connections(&direct).await.map_err(|e| {
                    ...
                    crate::integrations::composio::ops::report_composio_op_error(
                        "list_connections",
                        &rendered,
                    );
    ...
    async fn api_key(&self) -> tinybus::Result<Option<String>> {
        ...
        let stored = crate::security::credentials::get_composio_api_key(config.as_ref())
            .ok()
            .flatten();
...
impl ComposioCallbacks {
    ...
    async fn live_config(&self) -> Result<Config, String> {
        crate::config::rpc::reload_config_from_paths(&self.0.config_path, &self.0.workspace_dir)
            .await
    }
...
impl RuntimeCallbacks {
    ...
    async fn scheduler_policy(&self) -> tinybus::Result<(String, Option<String>)> {
        ...
        Ok(match crate::cron::scheduler_gate::gate::current_policy() {
    ...
    async fn extract_spacy(&self, text: String) -> tinybus::Result<SpacyResponse> {
        ...
        let started = std::time::Instant::now();
        let response = crate::runtime::python_server::extract_spacy(&self.0, &text)
            .await
            .map_err(method_error);
        log::debug!(
...
fn into_domain_event(event: MemoryEvent) -> Option<crate::core::events::DomainEvent> {
    ...
    Some(match event {
        ...
        MemoryEvent::LocalModelUnavailable { origin } => {
            crate::memory::tree::health::user_error::publish_local_model_unavailable_user_error(
                &origin,
            );
            return None;
        }
        ...
        MemoryEvent::StoreCorruptQuarantined {
            ...
            crate::memory::tree::health::user_error::publish_store_corrupt_user_error(
                &origin,
